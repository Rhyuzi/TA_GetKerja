<?php
Class ReportLowongan extends CI_Controller{
    ...
    function index(){
        ...
        // mencetak string 
        $pdf -> Cell(190,7,'Web Aplikasi GetKerja',0,1,'C');
        ...
        $pdf -> Cell(190,30,'Daftar Lowongan',0,1,'C');


        $pdf->SetFont('Arial','B',10);
        $pdf->Cell(20,6,'NOID',1,0);
        $pdf->Cell(55,6,'NAMA PERUSAHAAN ',1,0);
        $pdf->Cell(37,6,'KOTA',1,0);
        $pdf->Cell(57,6,'PROVINSI',1,1);

        $pdf->SetFont('Arial','',10);

        $perusahaan = $this->db->get('lowongan_kerja')->result();
        foreach ($perusahaan as $row){
            $pdf->Cell(20,6,$row->noid,1,0);
            $pdf->Cell(55,6,$row->nama_perusahaan,1,0);
            $pdf->Cell(37,6,$row->kota,1,0);
            $pdf->Cell(57,6,$row->provinsi,1,1);
        }
